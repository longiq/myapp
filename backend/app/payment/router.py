...
from app.payment.schemas import (
    FrontendPaymentIntentIn, CustomerCreateIn, CardDetailsIn, CustomerOut,
    PaymentIntentCreateIn, PaymentConfirmIn, ChargeCustomerIn, PaymentOut,
    RefundIn, RefundOut,
)
...
def _customer_out(r) -> CustomerOut:
    return CustomerOut(customer_id=r.customer_id, email=r.email, name=r.name,
                       payment_method_id=r.payment_method_id, metadata=r.metadata)
...
def _payment_out(r) -> PaymentOut:
    return PaymentOut(
        ...
        amount=r.amount, currency=r.currency, amount_display=r.amount_display,
        customer_id=r.customer_id, payment_method_id=r.payment_method_id,
        client_secret=r.client_secret, metadata=r.metadata,
    )
...
def create_customer_endpoint(body: CustomerCreateIn):
    ...
    if body.address:
        a = body.address
        address = Address(line1=a.line1, city=a.city, country=a.country,
                          line2=a.line2, postal_code=a.postal_code, state=a.state)
    return _customer_out(create_customer(CustomerData(
...
def attach_pm_endpoint(customer_id: str, body: CardDetailsIn):
    card = CardDetails(number=body.number, exp_month=body.exp_month,
...
def create_intent_endpoint(body: PaymentIntentCreateIn):
    return _payment_out(create_payment_intent(
        amount=body.amount, currency=body.currency, customer_id=body.customer_id,
        payment_method_id=body.payment_method_id, description=body.description,
        metadata=body.metadata, confirm=body.confirm,
    ))
...
def charge_endpoint(body: ChargeCustomerIn):
    return _payment_out(charge_customer(
...
def refund_endpoint(body: RefundIn):
    ...
    return RefundOut(refund_id=r.refund_id, payment_intent_id=r.payment_intent_id,
